...
public interface IHasName<T extends TileEntity> {
    ...
    void renderName(final T entityIn, final MatrixStack matrixStackIn, final IRenderTypeBuffer bufferIn, final int packedLightIn);

    default boolean canRenderName(final T entityIn) {
        return isWithinDistanceToRenderName(entityIn, 6.0D);
    }

    static boolean isWithinDistanceToRenderName(final TileEntity entityIn, final double dis) {
    ...
    static void renderNameplate(final TileEntity entityIn, final ITextComponent displayNameIn, final float offsetY, final MatrixStack matrixStackIn, final IRenderTypeBuffer bufferIn, final int packedLightIn) {
        final Minecraft mc = Minecraft.getInstance();
        final EntityRendererManager renderManager = mc.getEntityRenderDispatcher();
