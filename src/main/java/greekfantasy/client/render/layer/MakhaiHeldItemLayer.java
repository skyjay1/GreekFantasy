...
public class MakhaiHeldItemLayer<T extends MakhaiEntity> extends LayerRenderer<T, MakhaiModel<T>> {
  ...
  public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entity,
    ...
    if (!entity.isInvisible()) {
      ItemStack left = entity.getItemStackFromSlot(EquipmentSlotType.OFFHAND);
      ItemStack right = entity.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
      // left hand items
      if(!left.isEmpty()) {
//        renderItem(matrixStackIn, bufferIn, packedLightIn, entity, getEntityModel().bipedLeftArm, left, 0);
        renderItem(matrixStackIn, bufferIn, packedLightIn, entity, getEntityModel().bipedRightArm2, left, 0, 0);
        renderItem(matrixStackIn, bufferIn, packedLightIn, entity, getEntityModel().bipedLeftArm2, left, -0.125F, 0);
      }
      // right hand items
      if(!right.isEmpty()) {
//        renderItem(matrixStackIn, bufferIn, packedLightIn, entity, getEntityModel().bipedRightArm, right, 0);
        renderItem(matrixStackIn, bufferIn, packedLightIn, entity, getEntityModel().bipedLeftArm3, right, 0, 0);
        renderItem(matrixStackIn, bufferIn, packedLightIn, entity, getEntityModel().bipedRightArm3, right, -0.125F, -90F);
      }
    }
  }
  ...
  private void renderItem(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entity,
    ...
    matrixStackIn.push();
    // transforms
    arm.translateRotate(matrixStackIn);
    ...
    Minecraft.getInstance().getItemRenderer().renderItem(entity, item, ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, 
