...
public class GorgonModel<T extends Gorgon> extends DrakainaModel<T> {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        createSnakeLayers(lowerHair, 3.8F, (float) Math.PI / (LOWER_HAIR_COUNT * 0.5F), CubeDeformation.NONE);
        createSnakeLayers(middleHair, 2.25F, (float) Math.PI / (MIDDLE_HAIR_COUNT * 0.5F), CubeDeformation.NONE);
        createSnakeLayers(upperHair, 1.25F, (float) Math.PI / (UPPER_HAIR_COUNT * 0.5F), CubeDeformation.NONE);
     ...
     */
    private static PartDefinition createSnakeLayers(final PartDefinition root, final float radius,
        ...
        for (double angle = 0.0D, count = 1.0D; angle < Math.PI * 2; angle += deltaAngle) {
            ...
            final PartDefinition holder = root.addOrReplaceChild(HOLDER + String.valueOf((int)count), CubeListBuilder.create(), PartPose.ZERO);
            final PartDefinition snake = createSnakeLayer(holder, cubeDeformation, ptX, -0.5F, ptZ, 0, angY, 0, 46, 52);
            count++;
        }
        return root;
    }

    private static PartDefinition createSnakeLayer(final PartDefinition root, final CubeDeformation cubeDeformation,
    ...
    private void setupSnakeAnim(final List<ModelPart> list, final float ticks, final float baseAngleX) {
        int i = 0;
