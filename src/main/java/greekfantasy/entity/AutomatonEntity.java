...
public class AutomatonEntity extends MonsterEntity {
  
  private static final DataParameter<Byte> STATE = EntityDataManager.createKey(AutomatonEntity.class, DataSerializers.BYTE);
  private static final String KEY_STATE = "AutomatonState";
  ...
  public AutomatonEntity(final EntityType<? extends AutomatonEntity> type, final World worldIn) {
    super(type, worldIn);
  ...
  public static AutomatonEntity spawnGeryon(final World world, final BlockPos pos, final float yaw) {
    AutomatonEntity entity = GFRegistry.AUTOMATON_ENTITY.create(world);
  ...
  protected void registerGoals() {
    super.registerGoals();
    this.goalSelector.addGoal(0, new AutomatonEntity.SpawningGoal());
    this.goalSelector.addGoal(1, new AutomatonEntity.SmashAttackGoal(SMASH_RANGE, 210));
    this.goalSelector.addGoal(2, new AutomatonEntity.MeleeAttackGoal(this, 1.0D, false));
...
//    this.targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(this, IronGolemEntity.class, false));
  ...
  public boolean isSmashAttack() { return getState() == SMASH; }
  
  public void setSmashAttack(final boolean smash) { setState(smash ? SMASH : NONE); }
  ...
  public void handleStatusUpdate(byte id) {
    ...
    case SMASH_CLIENT:
      ...
      final List<Entity> targets = this.getEntityWorld().getEntitiesWithinAABBExcludingEntity(AutomatonEntity.this, AutomatonEntity.this.getBoundingBox().grow(SMASH_RANGE, SMASH_RANGE / 2, SMASH_RANGE));
  ...
  class SpawningGoal extends Goal {
    ...
    @Override
    public boolean shouldExecute() { return AutomatonEntity.this.isSpawning(); }

    @Override
    public void tick() { AutomatonEntity.this.getNavigator().clearPath(); }
  }
  ...
  class MeleeAttackGoal extends net.minecraft.entity.ai.goal.MeleeAttackGoal {
    ...
    protected void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
      if(AutomatonEntity.this.hasNoCooldown()) {
    ...
    protected void func_234039_g_() {
      ...
      AutomatonEntity.this.setAttackCooldown();
    }
  }
  ...
  class SmashAttackGoal extends Goal {
    ...
    public void startExecuting() {
      AutomatonEntity.this.setSmashAttack(true);
      AutomatonEntity.this.getNavigator().getPathToEntity(AutomatonEntity.this.getAttackTarget(), 0);
    }
    ...
    public boolean shouldExecute() {
      ...
      return AutomatonEntity.this.getAttackTarget() != null && AutomatonEntity.this.isNoneState()
          && AutomatonEntity.this.hasNoCooldown()
          && AutomatonEntity.this.getDistanceSq(AutomatonEntity.this.getAttackTarget()) < (range * range);      
    }
    
    @Override
    public void tick() {
      AutomatonEntity.this.getNavigator().clearPath();
      AutomatonEntity.this.getLookController().setLookPositionWithEntity(AutomatonEntity.this.getAttackTarget(), AutomatonEntity.this.getHorizontalFaceSpeed(), AutomatonEntity.this.getVerticalFaceSpeed());
      if(AutomatonEntity.this.smashTime >= AutomatonEntity.MAX_SMASH_TIME) {
        // notify client (spawns particles around entities)
        AutomatonEntity.this.getEntityWorld().setEntityState(AutomatonEntity.this, AutomatonEntity.SMASH_CLIENT);
        // get a list of nearby entities and use smash attack on each one
        AutomatonEntity.this.getEntityWorld().getEntitiesWithinAABBExcludingEntity(AutomatonEntity.this, AutomatonEntity.this.getBoundingBox().grow(range, range / 2, range))
          .forEach(e -> AutomatonEntity.this.useSmashAttack(e));
        AutomatonEntity.this.setAttackCooldown();
        // finish task
        this.resetTask();
      }
    ...
    public boolean shouldContinueExecuting() { 
      return AutomatonEntity.this.isSmashAttack() && AutomatonEntity.this.getAttackTarget() != null 
              && AutomatonEntity.this.getDistanceSq(AutomatonEntity.this.getAttackTarget()) < (range * range); 
    }
    
    @Override
    public void resetTask() {
      AutomatonEntity.this.setSmashAttack(false);
      AutomatonEntity.this.smashTime = 0;
